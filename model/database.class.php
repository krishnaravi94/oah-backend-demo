...
class Database
{
    ...
    public function update($table, $data = array(), $conditionField = array())
    {
        // self::$counter++;
        //Make sure the array isn't empty
        if (empty($data)) {
        ...
        $fieldValueChanges = array();
        $conditionValueChanges = array();
        foreach ($data as $field => $value) {
            $fieldValueChanges[] = $field . "=" . "'" . $value . "'";
        }
        foreach ($conditionField as $field => $value){

            $conditionValueChanges[] = $field."=".$value;
        }
        $sql.= implode(', ',$fieldValueChanges).' WHERE '.implode($conditionValueChanges);
        // print_r($sql);
